<?php
/**
 * {{SITE_NAME}} — Contact Form Handler
 * Drop into public_html/ alongside the HTML files and set the contact form's
 * action="form-handler.php" if you prefer self-hosting over Formspree.
...
const RATE_FILE   = '/tmp/form_rate_portfolio.json';
...
// ── Rate limiting (per IP, 30 s window) ─────────────────────────────────────
...
if ($origin !== '' && $origin !== 'https://' . SITE_DOMAIN && $origin !== 'http://' . SITE_DOMAIN) {
...
$type    = sanitize($body['project_type'] ?? 'website');
...
$to       = RECIPIENT;
$sub      = SITE_NAME . " — {$type} enquiry from {$name}";
